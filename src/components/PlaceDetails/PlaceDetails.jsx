  ...
  return (
    <div className="card border-light p-2">
      ...
      <div className="card-text d-flex justify-content-between mt-1">
        ...
        <div>
          <span className="review__num">{place.num_reviews}</span> review
          {place.num_reviews > 1 && "s"}
        </div>
      </div>
      <div className="card-text d-flex justify-content-start mt-2">
        <div className="price">Price</div>
        <div className="ms-2">{place.price_level}</div>
      </div>
      <div className="card-text d-flex justify-content-start mt-2">
        <div className="price">Ranking</div>
        <div className="ms-2">{place.ranking}</div>
      </div>
      <div className="mt-1">
        {place?.cuisine?.map(({ name }) => (
          <div className="cuisin__name mb-2 me-2 px-2 py-1 rounded-pill">
            {name}
          </div>
        ))}
      </div>
      ...
      <div>
        <button
          ...
          className="btn btn-blue mt-2 ms-1"
        >
          Add Event
        </button>
        <AddEventModal
